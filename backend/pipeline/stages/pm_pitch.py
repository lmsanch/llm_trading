...
TARGET_MODELS = ContextKey("target_models")
...
class PMPitchStage(Stage):
    ...
    async def execute(self, context: PipelineContext) -> PipelineContext:
        ...
        if not research_pack_a and not research_pack_b:
            ...
            research_pack_b = (
                research_pack_a  # Use same pack for both if only one provided
            )
    ...
    async def _generate_pm_pitches(
        ...
        prompt = self._build_pm_prompt(
            research_pack_a, research_pack_b, market_metrics, current_prices
        )
...
Return ONLY this JSON, no other text.""",
                    ...
                    retry_resp = await query_pm_models(
                        retry_messages, model_keys=[model_key]
                    )
                    if retry_resp is None:
                        retry_content = None
                    else:
                        model_resp = retry_resp.get(model_key)
                        if model_resp is None:
                            retry_content = None
                        else:
                            retry_content = model_resp.get("content")
                    if retry_content:
                        try:
                ...
                if pitch:
                    ...
                    print(
                        f"   Instrument: {pitch['selected_instrument']} | Direction: {pitch['direction']}"
                    )
                    print(
                        f"   Conviction: {pitch['conviction']} | Horizon: {pitch['horizon']}"
                    )
                else:
                    model_info = REQUESTY_MODELS[model_key]
                    print(
    ...
    def _build_pm_prompt(
        ...
        research_text_a = self._format_research_pack(
            research_pack_a, "Perplexity Research"
        )
...
Return as valid JSON only, no markdown formatting."""
    ...
    def _format_research_pack(self, research_pack: Dict[str, Any], label: str) -> str:
        ...
        if narratives:
            ...
            for i, narrative in enumerate(narratives[:3], 1):
                ...
                lines.append(
                    f"   Why it moves prices: {narrative.get('why_it_moves_prices_this_week', '')}"
                )
                lines.append(
                    f"   Key catalysts: {narrative.get('key_catalysts_next_7d', [])}"
                )
        ...
        lines.append("\n**Consensus Map:**")
        lines.append(
            f"  Street consensus: {consensus.get('street_consensus', 'Unknown')}"
        )
        lines.append(f"  Positioning: {consensus.get('positioning_guess', 'Unknown')}")
        lines.append(
            f"  Priced for: {consensus.get('what_market_is_priced_for', 'Unknown')}"
        )
        lines.append(
            f"  Surprise would be: {consensus.get('what_would_surprise', 'Unknown')}"
        )
        ...
        if candidates:
            ...
            for candidate in candidates[:5]:
                lines.append(
                    f"- {candidate.get('instrument', 'Unknown')}: {candidate.get('directional_bias', 'Unknown')}"
                )
    ...
    def _format_market_metrics(self, market_metrics: Dict[str, Any]) -> str:
        ...
        if correlation_matrix:
            ...
            key_pairs = [("SPY", "QQQ"), ("SPY", "TLT"), ("GLD", "TLT"), ("USO", "SPY")]
    ...
    def _format_graph_digest(self, digest: Dict[str, Any]) -> str:
        ...
        if top_edges:
            ...
            for i, edge in enumerate(top_edges, 1):
                ...
                lines.append(
                    f"{i}. [{source_type}] {source_label} → {relation} → [{target_type}] {target_label}"
                )
                lines.append(
                    f"   Impact: {impact_score:.3f} (Strength: {strength:.2f}, Confidence: {confidence:.2f}, Sign: {sign} {sign_indicator})"
                )
            ...
            for ticker, subgraph in sorted_assets:
                ...
                if direct_drivers:
                    ...
                    for driver in direct_drivers:
                        ...
                        driver_line = (
                            f"    - {label} {sign_indicator} (Impact: {impact:.3f})"
                        )
                        if condition:
    ...
    def _parse_pm_pitch(self, content: str, model_key: str) -> Dict[str, Any] | None:
        """
        ...
        _agent_log(
            {
                ...
                "data": {"model": model_key, "content_len": len(content or "")},
            }
        )
        # endregion
            ...
            _agent_log(
                {
                    ...
                    "data": {"model": model_key, "preview": (content or "")[:160]},
                }
            )
            # endregion
        ...
        for attempt_name, attempt_str in parse_attempts:
            try:
                ...
                if attempt_name != "original":
                    print(
                        f"  ✓ Parsed JSON from {model_key} after repair: {attempt_name}"
                    )
                else:
            ...
            _agent_log(
                {
                    ...
                    "data": {"model": model_key, "error": str(e), "keyword": e.keyword},
                }
            )
            # endregion
            print(f"  {str(e)}")
        ...
        if direction == "FLAT":
            if entry_policy["mode"] not in ["NONE", None]:
                raise ValueError(
                    f"FLAT trades must have entry_policy.mode='NONE' (got: {entry_policy['mode']})"
                )
        else:
        ...
        if direction == "FLAT":
            ...
            if pitch.get("risk_profile") not in [None, "NONE"]:
                raise ValueError(
                    f"FLAT trades must not have risk_profile (got: {pitch.get('risk_profile')})"
                )
            if pitch.get("exit_policy") not in [None, {}]:
                exit_policy = pitch.get("exit_policy", {})
                if exit_policy and (
                    exit_policy.get("stop_loss_pct")
                    or exit_policy.get("take_profit_pct")
                    or exit_policy.get("time_stop_days")
                ):
                    raise ValueError(
                        f"FLAT trades must not have exit_policy with stop_loss/take_profit/time_stop"
                    )
            # FLAT entry_policy should be NONE or omitted
            if entry_policy.get("mode") not in ["NONE", None]:
                raise ValueError(
                    f"FLAT trades must have entry_policy.mode='NONE' (got: {entry_policy.get('mode')})"
                )
        else:
            # For LONG/SHORT, risk_profile and exit_policy are required
        ...
        if pitch["direction"] == "LONG" and conviction <= 0:
            print(
                f"  ⚠️  LONG direction must have positive conviction, got {conviction}"
            )
            return None

        if pitch["direction"] == "SHORT" and conviction >= 0:
            print(
                f"  ⚠️  SHORT direction must have negative conviction, got {conviction}"
            )
            return None
        ...
        _agent_log(
            {
                ...
                "data": {"model": model_key, "fields": list(pitch.keys())},
            }
        )
        # endregion
        return pitch
