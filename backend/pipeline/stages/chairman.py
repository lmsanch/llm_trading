...
class ChairmanStage(Stage):
    ...
    async def execute(self, context: PipelineContext) -> PipelineContext:
        ...
        chairman_decision = await self._generate_chairman_decision(
            pm_pitches, peer_reviews, label_to_model
        )
...
{{
  ...
  "monitoring_plan": {{
    ...
    "watch_conditions": ["Stop loss hit", "Take profit hit", "Time stop reached", "Event-based exit"]
  }},
  "timestamp": "ISO8601"
}}
...
### GUIDELINES:
 ...
 1. SELECT TRADE:
    - Pick ONE instrument from PM recommendations
    - Choose direction (LONG/SHORT/FLAT) based on consensus
    - Set horizon (1d or 3d or 1w) based on timing
...
3. DISSENT SUMMARY:
 ...
 4. MONITORING PLAN:
    ...
    - Watch conditions: Stop loss hit, take profit hit, time stop reached, event-based exit
...
Return as valid JSON only, no markdown formatting."""
    ...
    def _fallback_decision(self, pm_pitches: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        valid_pitches = [
            p for p in pm_pitches if p.get("direction") in ["LONG", "SHORT"]
        ]
