        ...
        try {
            ...
            if (data.history) {
                ...
                for (const date of dates) {
                    ...
                    if (!latestPerplexity && pEntry && pEntry.report_ids && pEntry.report_ids.length > 0) {
                        latestPerplexity = { id: pEntry.report_ids[0], date };
                        break;
                    }
                }
                ...
                if (latestPerplexity) {
                    try {
                        const res = await fetch(`/api/research/report/${latestPerplexity.id}`);
                        if (res.ok) {
        ...
        <Card className="w-full">
            ...
            <CardContent className="p-4">
                ...
                <div className="mt-6 pt-6 border-t">
                    <h4 className="text-sm font-semibold mb-4 text-muted-foreground">Latest Research</h4>
                    {loadingReport ? (
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center justify-between text-xs text-muted-foreground px-1">
                                <span className="flex items-center gap-2">
                                    ...
                                    Latest: {new Date(latestReport.created_at).toLocaleString()}
                                </span>
                            ...
                            <div>
                                <ResearchPackCard
                                    data={{
                                        source: latestReport.provider,
                                        model: latestReport.model,
                                        natural_language: latestReport.natural_language,
                                        ...(latestReport.structured_json || {})
                                    }}
                                />
                            </div>
                        </div>
